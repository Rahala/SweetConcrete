...
public class ConcreteSignTileEntityRenderer extends TileEntityRenderer<ConcreteSignTileEntity> {
    private static final Map<DyeColor, ResourceLocation> signTextures =
            Arrays.stream(DyeColor.values()).collect(Collectors.toMap(c -> c, c -> new ResourceLocation("textures/block/" + c.getTranslationKey() + "_concrete.png")));

    private final SignModel model = new SignModel();

    public ConcreteSignTileEntityRenderer() {
        model.getSignStick().isHidden = true;
    }

    private ResourceLocation getSignTexture(Block block) {
        return signTextures.get(((ConcreteSignBlock) block).dyeColor);
    }

    @Override
    public void render(ConcreteSignTileEntity tileEntity, double x, double y, double z, float partialTicks, int destroyStage) {
